...
# Copyright 2026 DearEIS developers
#
...
class ProjectTab:
    def __init__(self, uuid: str, tab_bar: int, state):
        ...
        with dpg.tab(
            ...
            for tab in self.tab_lookup.values():
                ...
                item_handler: Tag = dpg.generate_uuid()
    ...
    def resize(self, width: int, height: int):
        ...
        tab: Tag = dpg.get_value(self.tab_bar)
    ...
    def select_next_tab(self):
        current_tab: Tag = dpg.get_value(self.tab_bar)
    ...
    def get_active_test(self) -> Optional[KramersKronigResult]:
    ...
    def get_next_test_result(self) -> Optional[KramersKronigResult]:
        return self.kramers_kronig_tab.get_next_result()

    def get_previous_test_result(self) -> Optional[KramersKronigResult]:
    ...
    def select_test_result(self, test: KramersKronigResult, data: DataSet):
    ...
    def get_test_settings(self) -> KramersKronigSettings:
        return self.kramers_kronig_tab.get_settings()

    def set_test_settings(self, settings: KramersKronigSettings):
    ...
    def show_admittance_plots(self) -> bool:
        return {
            Context.DATA_SETS_TAB: self.data_sets_tab.show_admittance_plots,
            Context.KRAMERS_KRONIG_TAB: self.kramers_kronig_tab.show_admittance_plots,
            Context.FITTING_TAB: self.fitting_tab.show_admittance_plots,
            Context.SIMULATION_TAB: self.simulation_tab.show_admittance_plots,
        }.get(self.get_active_context(), lambda: False)()

    def start_loading_simulation_as_data_set(self):
        self.simulation_tab.settings_table.show_noise_addition_window(
