...
class Bode(Plot):
    def __init__(self, width: int = -1, height: int = -1, *args, **kwargs):
        ...
        with dpg.plot(
            ...
            self._y_axis_1: int = dpg.add_plot_axis(dpg.mvYAxis, label="log |Z|")
            self._y_axis_2: int = dpg.add_plot_axis(dpg.mvYAxis, label="-phi (°)")
        dpg.bind_item_theme(self._plot, themes.plot)
        dpg.bind_item_handler_registry(self._plot, self._item_handler)

    @classmethod
    def duplicate(Class, original: "Bode", *args, **kwargs) -> "Bode":
        copy: "Bode" = Class(*args, **kwargs)
    ...
    def adjust_limits(self):
        ...
        if x_min is None:
            ...
            dx: float = 0.1
            x_min = floor(x_min / dx) * dx - dx
            x_max = ceil(x_max / dx) * dx + dx
            dy: float = 0.1
            y1_min = floor(y1_min / dy) * dy - dy
            y1_max = ceil(y1_max / dy) * dy + dy
            n: int = 5
    ...
    def copy_limits(self, other: "Bode"):
        src: int
        dst: int
        for src, dst in zip(
            [
...
class BodeMagnitude(Plot):
    def __init__(self, width: int = -1, height: int = -1, *args, **kwargs):
        ...
        with dpg.plot(
            ...
            self._x_axis: int = dpg.add_plot_axis(dpg.mvXAxis, label="log f")
            self._y_axis: int = dpg.add_plot_axis(dpg.mvYAxis, label="log |Z|")
        dpg.bind_item_theme(self._plot, themes.plot)
        dpg.bind_item_handler_registry(self._plot, self._item_handler)

    @classmethod
    def duplicate(Class, original: "BodeMagnitude", *args, **kwargs) -> "BodeMagnitude":
        copy: "BodeMagnitude" = Class(*args, **kwargs)
        for kwargs in original.get_series():
            copy.plot(**kwargs)
        return copy
    ...
    def adjust_limits(self):
        ...
        if x_min is None:
            ...
            x_max = ceil(x_max / dx) * dx + dx
            dy: float = 0.1
            y_min = floor(y_min / dy) * dy - dy
            y_max = ceil(y_max / dy) * dy + dy
        dpg.split_frame()
        dpg.set_axis_limits(self._x_axis, ymin=x_min, ymax=x_max)
    ...
    def copy_limits(self, other: "BodeMagnitude"):
        src: int
        dst: int
        for src, dst in zip(
            [
...
class BodePhase(Plot):
    def __init__(self, width: int = -1, height: int = -1, *args, **kwargs):
        ...
        with dpg.plot(
            ...
            self._x_axis: int = dpg.add_plot_axis(dpg.mvXAxis, label="log f")
            self._y_axis: int = dpg.add_plot_axis(dpg.mvYAxis, label="-phi (°)")
        dpg.bind_item_theme(self._plot, themes.plot)
        dpg.bind_item_handler_registry(self._plot, self._item_handler)

    @classmethod
    def duplicate(Class, original: "BodePhase", *args, **kwargs) -> "BodePhase":
        copy: "BodePhase" = Class(*args, **kwargs)
        for kwargs in original.get_series():
            copy.plot(**kwargs)
        return copy
    ...
    def copy_limits(self, other: "BodePhase"):
        src: int
        dst: int
        for src, dst in zip(
            [
