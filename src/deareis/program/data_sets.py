...
def load_simulation_as_data_set(*args, **kwargs):
    ...
    f: Frequencies = simulation.get_frequencies()
    Z: ComplexImpedances = simulation.circuit.get_impedances(f)

    noise_pct: float = max((0.0, kwargs.get("noise_pct", 0.0)))
    if noise_pct > 0.0:
        sd: NDArray[float64] = noise_pct / 100 * abs(Z)
        rs: RandomState = RandomState(seed=randint(2**32-1))

        Z_noise: ComplexImpedances = zeros(Z.shape, dtype=Z.dtype)
        Z_noise.real = rs.normal(0, sd)
        Z_noise.imag = rs.normal(0, sd)
        Z += Z_noise
